...
namespace play_math_vector3
{
	...
	r2tm::DoFunctionT Length::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			OUT_SUBJECT( "피타고라스의 정리" );
			OUT_COMMENT( "밑변 = a, 높이 = b, 빗변 = c" );
			OUT_COMMENT( "a^2 + b^2 = c^2" );
			OUT_COMMENT( "sqrt( a^2 + b^2 ) = c" );

			LF();

			OUT_SUBJECT( "피타고라스의 정리를 3차원으로 확장" );
			OUT_COMMENT( "Vec3 A 의 xy 성분만으로 Vec2 B 를 만든다." );
			OUT_COMMENT( "B의 길이를 구한다." );
			OUT_COMMENT( "B의 길이 = 높이, A의 z 성분 = 밑변 으로 피타고라스의 정리 적용." );

			LF();

			OUT_COMMENT( "A의 길이 = sqrt( B의 길이^2 + z^2 )" );
			OUT_COMMENT( "||A|| = sqrt( sqrt( x^2 + y^2 )^2 + z^2 )" );
			OUT_COMMENT( "||A|| = sqrt( x^2 + y^2 + z^2 )" );
	...
	r2tm::DoFunctionT Normalize::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			OUT_SUBJECT( "벡터의 정규화" );
			OUT_COMMENT( "벡터의 방향은 유지 하면서 길이를 1로 만든다." );
			OUT_COMMENT( "벡터의 각 성분을 길이로 나눈다." );
	...
	r2tm::DoFunctionT Dot_Product_1::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			OUT_SUBJECT( "내적은 두 벡터가 같은 방향을 향하는지" );
			OUT_SUBJECT( "또는 정렬되어 있는지를 나타내는 척도." );

			LF();

			OUT_SUBJECT( "내적의 대수적 정의" );
			OUT_COMMENT( "A dot B = AxBx + AyBy + AzBz + AwBw" );
			...
			{
				OUT_SUBJECT( "( Y x Y ), ( Y x -Y ) : 앞/뒤 방향 알 수 있다 : 수평에 1" );
			...
			{
				OUT_SUBJECT( "( Y x X ), ( Y x -X ) : 좌/우 방향 알 수 없다. : 수직에 0" );
	...
	r2tm::DoFunctionT Dot_Product_2::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			{
				OUT_SUBJECT( "지향성 판단을 원한다면 단위 벡터를 사용해라." );

				SS();

				OUT_COMMENT( "단위 벡터가 아닌 경우" );
			...
			{
				OUT_SUBJECT( "기준 벡터가 단위 벡터 라면 내적의 결과는 투영된 벡터의 길이" );
	...
	r2tm::DoFunctionT Dot_Product__Scalar_Projection::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			{
				OUT_SUBJECT( "스칼라 투영" );
				OUT_COMMENT( "벡터 B가 벡터 A 방향으로 가지는 길이를 나타내는 스칼라 값" );
				OUT_COMMENT( "벡터 B의 끝점에서 벡터 A에 수선을 내렸을 때 A에 드리운 그림자의 길이" );
				OUT_COMMENT( "compbA = ||B|| * cos(θ)" );
				OUT_COMMENT( "\t" "θ = A 와 B의 각도" );

				LF();

				OUT_SUBJECT( "내적의 기하학적 정리" );
				OUT_COMMENT( "A dot B = ||A|| * ||B|| * cos(θ)" );

				LF();

				OUT_SUBJECT( "cos(θ) 를 기준으로 공식 변환" );
				OUT_COMMENT( "cos(θ) = ( A dot B ) / ( ||A|| * ||B|| )" );

				LF();

				OUT_SUBJECT( "적용" );
				OUT_COMMENT( "compbA = ||B|| * ( ( A dot B ) / ( ||A|| * ||B|| ) )" );
				OUT_COMMENT( "compbA = ( A dot B ) / ||A||" );
			}
	...
	r2tm::DoFunctionT Dot_Product__Vector_Projection::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			{
				OUT_SUBJECT( "벡터 투영" );
				OUT_COMMENT( "스칼라 투영의 길이를 가지면서" );
				OUT_COMMENT( "기준 벡터와 같은 방향을 향하는 벡터" );

				LF();

				OUT_SUBJECT( "공식 유도" );
				OUT_COMMENT( "1. 벡터의 길이" );
				OUT_COMMENT( "\t" "스칼라 투영으로 얻을 수 있다." );
				OUT_COMMENT( "\t" "compbA = ( A dot B ) / ||A||" );
				OUT_COMMENT( "2. 벡터의 방향" );
				OUT_COMMENT( "\t" "기준 벡터를 단위 벡터로 만들면 된다." );
				OUT_COMMENT( "\t" "NA = A / ||A||" );
				OUT_COMMENT( "3. NA * 길이" );
				OUT_COMMENT( "\t" "projaB = ( ( A dot B ) / ||A|| ) * ( A / ||A|| )" );
				OUT_COMMENT( "\t" "projaB = ( ( A dot B ) / ||A|| ) * ( 1 / ||A|| ) * A" );
				OUT_COMMENT( "\t" "projaB = ( ( A dot B ) / ( ||A|| * ||A|| ) ) * A" );

				LF();

				OUT_SUBJECT( "projaB = ( A dot B / ||A||^2 ) * A" );
	...
	r2tm::DoFunctionT Angle_Between_Two_Vectors::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			OUT_SUBJECT( "두 벡터의 각은 내적을 이용해 구한다." );
...

			{
				OUT_SUBJECT( "내적의 기하학적 정리" );
				OUT_COMMENT( "A dot B = ||A|| * ||B|| * cos(θ)" );
				OUT_COMMENT( "\t" "||A|| = 벡터 A 의 크기" );
				OUT_COMMENT( "\t" "벡터의 크기 = sqrt( x^2 + y^2 + z^2 )" );

				LF();

				OUT_SUBJECT( "각을 구하기 위한 공식 변환" );
				OUT_COMMENT( "cos(θ) = ( A dot B ) / ( ||A|| * ||B|| )" );
				OUT_COMMENT( "θ = arccosine( ( A dot B ) / ( ||A|| * ||B|| ) )" );
				OUT_COMMENT( "\t" "arccosine/acos = cos의 역함수" );
			}
	...
	r2tm::DoFunctionT Angle_Between_Two_Unit_Vectors::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			OUT_SUBJECT( "두 단위 벡터의 각 계산은 단순해진다." );
...

			{
				...
				OUT_COMMENT( "cos(θ) = ( A dot B ) / ( ||A|| * ||B|| )" );
				OUT_COMMENT( "θ = arccosine( ( A dot B ) / ( ||A|| * ||B|| ) )" );
				OUT_COMMENT( "θ = arccosine( ( A dot B ) / ( 1 * 1 ) )" );
				OUT_COMMENT( "θ = arccosine( ( A dot B ) / 1 )" );
				OUT_COMMENT( "θ = arccosine( A dot B )" );
			}
	...
	r2tm::DoFunctionT Cross_Product::GetDoFunction() const
	{
		return []()->r2tm::eDoLeaveAction
		{
			...
			OUT_SUBJECT( "외적이란?" );
			OUT_COMMENT( "두 벡터를 바탕으로 두 벡터 모두에 수직인 새로운 벡터를 생성." );

			LF();

			OUT_SUBJECT( "외적의 오른손 법칙" );
			OUT_COMMENT( "오른손의 엄지를 제외한 4 손가락으로 첫 벡터 방향을 가리킨다." );
			OUT_COMMENT( "엄지는 수직으로" );
			OUT_COMMENT( "4 손가락을 두번째 벡터 방향으로 구부린다.( 이 과정에서 축 회전이 필요 )" );
			OUT_COMMENT( "엄지의 방향이 결과 벡터" );

			LF();

			OUT_SUBJECT( "교환 법칙이 성립하지 않는다." );

			LF();

			OUT_SUBJECT( "외적의 대수적 정의" );
			OUT_COMMENT( "A x B = ( AyBz - AzBy, AzBx - AxBz, AxBy - AyBx )" );
